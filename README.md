...
Хорошо бы вовлечь энтузиастов на стороне Яндекс, кто готов помочь или хотя бы пару раз созвониться чтобы подсказали.
2) Сценарий для Яндекс Cloud Functions (ServerLess) или написать узел Node-RED (ожидается поддержка Knative)
На стороне Яндекса нужно искать заинтересованное лицо - "спонсора", поскольку задача может быть достаточно трудоемкой.
3) Рассмотреть возможность использования Open Source проекта Open-Horizon для Edge сценариев в Я.Облаке
