...
Это название приклеелось к проекту и стало существенным улучшением для того,
как он назывался в первые несколько дней.
Часть «Node» отражает суть модели потокового программирования (поток/узел),
так и основную среду выполнения Node.JS.
Окончательного решения о том, что же означает часть «RED», принято так и не было.
Одним из предложений было «Rapid Event Developer» (быстрый разработчик событий),
но мы никогда не чувствовали себя обязанными что-либо формализовать.
Мы придерживаемся названия «Node-RED»."
```
Node-RED предоставляется в соответствии с условиями лицензии Apache 2.0. Важно понимать и осознавать условия [этой лицензии](https://github.com/node-red/node-red/blob/master/LICENSE), но по предоставленной ссылке есть краткая выдержка с её основными свойствами: https://tldrlegal.com/license/apache-license-2.0-(apache-2.0).
Лицензия разрешает коммерческое использование, но надо учитывать что лицензия накладывает ряд ограничений. Основные ограничения: использования торговой марки (Trademark use) - нельзя искажать торговую марку Node-RED (принадлежащую OpenJS Foundation) и ограничение ответственности (Liability/Warranty), то есть участники проекта не могут быть привлечены к ответственности в причинении каких либо вероятных убытков.
...
Рассмотрим добавленные ноды. *Send data* - нод типа *Inject*, предназначен для ручного или автоматического запуска flow.
*Device payload* - нод-функция, позволяет выполнять JavaScript для обработки данных, проходящих через него.
*device* - *mqtt-out* нод с настраиваемым mqtt-подключением, предназначен для публикации данных в топик.
![Device flow](screenshots/device_flow.png)

Два раза кликаем по ноду *device*. 
...
Данный flow получился намного интереснее предыдущего, можем заметить новые ноды.
Так, *registry* - *mqtt-in* нод, принимает сообщения из указанного в настройках топика и передает их следующим нодам.
Измеряемые данные приходят в строке, поэтому применяется нод парсер *json*, может конвертировать JSON-объект в его JS-представление и обратно.
*Parsing JSON* и *JSON* возвращают полученные измерения уже в виде отдельных объектов, которые далее передаются нодам, отвечающим за элементы *Dashboard*.
